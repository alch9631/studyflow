...
- [ ] (feature, full-stack) Global search: a search affordance (reachable on mobile) that searches across courses, topics, and deadlines by name and jumps to the match. Server action or lightweight client filter over the user's own data (userId-scoped); keyboard accessible, with a clear empty state.
