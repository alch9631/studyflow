...
- [ ] (i18n, FOUNDATIONAL — scope to setup + German + core screens) German localization + language toggle: introduce i18n (next-intl or a lightweight typed dictionary — pick the lighter option that fits this app), wire a Language toggle (DE/EN) in Settings + the menu that persists, and default to the browser language. Translate the CORE surfaces to German: nav/bottom bar, Today, My Courses, course detail, Insights, and common buttons/toasts/empty-states. Leave less-common screens (catalog, import, settings detail) as follow-up `- [ ]` items the feeder can add. Keep English as the second locale; no hardcoded user-facing strings left in the translated screens.
