...
- [ ] (mobile) Install prompt / Add to Home Screen: capture `beforeinstallprompt` and show a dismissible "Install StudyFlow" nudge on Android/Chrome; on iOS Safari show a one-time "Add to Home Screen" hint (since iOS has no prompt API). Remember dismissal in localStorage; never nag.
