...
- [x] (hardening) Web-push reliability tests: cover the sender's edge cases without real network — 410/404 Gone prunes the subscription, malformed/expired subscription is skipped, multi-subscription fan-out, and the "push not configured" no-op path. Plus tests for the daily-reminder message builder (0 sessions, many sessions, hours formatting). Mock the web-push sender; follow the existing test style.
...
- [x] (robustness) Stop swallowing AI/server-action failures: the empty `catch {}` blocks in src/app/courses/actions.ts (reoptimizeCourse, importSyllabus, healCourse, analyzeModuleUpload) hide network errors, missing-API-key, and real bugs behind one generic "failed". Log the error server-side (console.error with context) and distinguish transient (network/timeout) from not-configured (no AI key) so the redirect `?msg=` reason / UI banner is accurate. Add tests for the not-configured path.
- [x] (perf) Scope the stats cache invalidation: today every course/topic/block mutation clears the whole stats cache (statsCache.ts), so the next Insights/Today read rescans all data. Invalidate per-userId (and per-course where cheap) instead of nuking everything. Keep the 30s TTL + write-invalidate behaviour; add a test that an unrelated user's cache entry survives a mutation.
- [x] (robustness) Push key-rollover resync: a PushSubscription created while VAPID keys were unset never receives deliveries once keys are added. Add a flag/version on the subscription (or a re-validate-on-next-load path) so subscriptions made in the unconfigured state get reactivated/re-synced when push becomes configured. Tests for the rollover path (mock the sender).
- [x] (cleanup) Remove stray `console.log` calls across src/ (keep intentional console.error in error paths) and update the outdated AI model ids in src/lib/syllabus.ts to current models. No behaviour change beyond logging hygiene; keep the env-gated AI no-op intact.
