...
- [ ] (perf) Scope the stats cache invalidation: today every course/topic/block mutation clears the whole stats cache (statsCache.ts), so the next Insights/Today read rescans all data. Invalidate per-userId (and per-course where cheap) instead of nuking everything. Keep the 30s TTL + write-invalidate behaviour; add a test that an unrelated user's cache entry survives a mutation.
